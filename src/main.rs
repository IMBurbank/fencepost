            ...
            if let Ok(mut settings) = serde_json::from_str::<serde_json::Value>(&contents) {
                ...
                if let Some(arr) = pre.as_array_mut() {
                    arr.push(serde_json::json!({
                        "matcher": "Edit|Write",
                        "hooks": [{"type": "command", "command": "FENCEPOST_RULES_OFF=\"$FENCEPOST_RULES_OFF\" fencepost edit || true"}]
                    }));
                    arr.push(serde_json::json!({
                        "matcher": "Bash",
                        "hooks": [{"type": "command", "command": "FENCEPOST_RULES_OFF=\"$FENCEPOST_RULES_OFF\" fencepost bash || true"}]
                    }));
                }
            ...
            match std::fs::read_to_string(&settings_path) {
                Ok(contents) => {
                    if contents.contains("fencepost edit") || contents.contains("fencepost bash") {
                        ...
                        eprintln!(
                            "      {{\"matcher\":\"Edit|Write\",\"hooks\":[{{\"command\":\"FENCEPOST_RULES_OFF=\\\"$FENCEPOST_RULES_OFF\\\" fencepost edit || true\"}}]}},"
                        );
                        eprintln!(
                            "      {{\"matcher\":\"Bash\",\"hooks\":[{{\"command\":\"FENCEPOST_RULES_OFF=\\\"$FENCEPOST_RULES_OFF\\\" fencepost bash || true\"}}]}}"
                        );
                        eprintln!("    ]}}}}");
                        ok = false;
                    }
